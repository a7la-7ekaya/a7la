...
let currentActiveSlideMain = 0; let slideIntervalTimerMain;
...
function buildSideList() {
    ...
    topicsData.forEach((topic, index) => {
        const itemHTML = `<div class="side-item ${index === currentActiveSlideMain ? 'active-slide' : ''}" onclick="changeSlideMain(${index})">
...
function changeSlideMain(index) { currentActiveSlideMain = index; renderHeroMain(index); resetSlideTimerMain(); }
function autoNextSlideMain() { currentActiveSlideMain = (currentActiveSlideMain + 1) % topicsData.length; renderHeroMain(currentActiveSlideMain); }
function startSlideTimerMain() { slideIntervalTimerMain = setInterval(autoNextSlideMain, 4000); }
function resetSlideTimerMain() { clearInterval(slideIntervalTimerMain); startSlideTimerMain(); }

const sliderSection = document.getElementById('sliderSection');
if(sliderSection) {
    sliderSection.addEventListener('mouseenter', () => clearInterval(slideIntervalTimerMain));
    sliderSection.addEventListener('mouseleave', startSlideTimerMain);
}
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (sliderContainer) {
        ...
        if (sideItems.length > 0) {
            let currentActiveSlideDynamic = 0;
            let slideIntervalTimerDynamic;
            ...
            async function extractImageFromMetaTags(url, itemElement, index) {
                try {
                    ...
                    if (index === currentActiveSlideDynamic) {
            ...
            sideItems.forEach((item, index) => {
                ...
                item.addEventListener('click', () => { changeSlideDynamic(index); });
            });

            function renderHeroDynamic(index) {
            ...
            function changeSlideDynamic(index) { currentActiveSlideDynamic = index; renderHeroDynamic(index); resetSlideTimerDynamic(); }
            function autoNextSlideDynamic() { currentActiveSlideDynamic = (currentActiveSlideDynamic + 1) % sideItems.length; renderHeroDynamic(currentActiveSlideDynamic); }
            function startSlideTimerDynamic() { slideIntervalTimerDynamic = setInterval(autoNextSlideDynamic, 4500); }
            function resetSlideTimerDynamic() { clearInterval(slideIntervalTimerDynamic); startSlideTimerDynamic(); }

            sliderContainer.addEventListener('mouseenter', () => clearInterval(slideIntervalTimerDynamic));
            sliderContainer.addEventListener('mouseleave', startSlideTimerDynamic);

            renderHeroDynamic(0);
            startSlideTimerDynamic();
        }
    }
});
...
$(document).ready(function() {
    ...
    Fancybox.bind('.content img:not(.smilies):not([src*="illiweb"]):not([src*="hits"])', {
        groupAll: true,
    ...
    $(document).on('click', '.fa_like_div .rep-button', function(e) {
        e.preventDefault();
        if(typeof _userdata === "undefined" || _userdata.session_logged_in != 1) { return; }
    ...
    if(topTags.length > 0) {
        ...
        topTags.forEach(function(tag) { 
            ...
            $('#flx-auto-tags .tags-container').append('<a href="' + searchUrl + '">#' + tag + '</a>'); 
        });
    }
